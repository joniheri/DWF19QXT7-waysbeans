      ...
      <div className="p-modal">
        <div className="w-modal">
          ...
          <form className="">
            ...
            <input name="name" placeholder="Full Name" className="input w-100" onChange={(e) => setName(e.target.value)} value={name} />
            <button className="btn btn-primary w-100 btn-modal mt-28" onClick={onRegister}>
              Register
            </button>
            <p className="text-center text-modal mt-21"> Already have an account..? {" "}
              <span className="half-bold cursor" onClick={onSwitch}>Klik Here </span>
            </p>
          </form>
        </div>
      </div>
      <Popup text="Please Complite Your Data!" type="danger" show={showPop} setShow={setShowPop} />
    </Modal>
  );
}
